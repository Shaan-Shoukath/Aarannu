      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            {org?.logo_url ? (
              <img src={org.logo_url} alt="" className="w-8 h-8 rounded-lg object-cover ring-1 ring-slate-200" />
            ) : (
            ...
            <div>
              <div className="flex items-center gap-2">
                <button onClick={() => navigate(`/org/${slug}/events`)} className="text-sm text-slate-400 hover:text-slate-700 transition cursor-pointer">
                  Events
                </button>
                <span className="text-slate-300">/</span>
                <h1 className="text-sm font-semibold text-slate-900 max-w-48 truncate">{event?.name}</h1>
                <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${isEnded ? "text-slate-500 bg-slate-50 border-slate-200" : "text-emerald-700 bg-emerald-50 border-emerald-200"}`}>
                  {event?.status}
                </span>
              </div>
              <p className="text-xs text-slate-400">/{org?.slug} · {userRole}</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => navigate("/org/new")} className="px-3 py-1.5 text-xs text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition cursor-pointer">
              Switch Org
            </button>
            <button onClick={handleSignOut} className="px-3 py-1.5 text-xs text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer">
              Sign Out
            </button>
          </div>
        </div>
