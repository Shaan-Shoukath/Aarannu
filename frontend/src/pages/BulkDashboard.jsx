...
export default function BulkDashboard() {
    ...
    (async () => {
      try {
        const [projRes, fieldsRes] = await Promise.all([
          fetch(`${BACKEND}/api/projects/${projectId}`, { headers: authHeaders() }),
          fetch(`${BACKEND}/api/form-fields/${projectId}`, { headers: authHeaders() }),
        ]);
        const projJson = await projRes.json();
        const fieldsJson = await fieldsRes.json();
        if (!projRes.ok) { setError(projJson.error || "Failed to load project"); setLoading(false); return; }
  ...
  const handleFetchSheet = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND}/api/sheets/fetch`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ sheetUrl: sheetUrl.trim(), gid: gid.trim() || undefined }),
      });
      const json = await res.json();
      if (!res.ok) { setError(json.error || "Failed to fetch sheet."); setFetching(false); return; }
      ...
      const fieldLabels = {};
      const fieldKeys = {};
      formFields.forEach(f => { fieldLabels[f.label?.toLowerCase()] = f.field_key; fieldKeys[f.field_key?.toLowerCase()] = f.field_key; });
      (json.headers || []).forEach(h => {
        ...
        else if (lower === "email" || lower === "email address") mapping[h] = "email";
        else if (lower === "photo" || lower === "photo url") mapping[h] = "photo";
      });
      setColumnMapping(mapping);
      setStep(2);
    } catch { setError("Network error fetching sheet."); }
  ...
  const updateMapping = (sheetCol, fieldKey) => {
    setColumnMapping(prev => {
      const next = { ...prev };
      if (!fieldKey) delete next[sheetCol]; else next[sheetCol] = fieldKey;
      return next;
    });
  };
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND}/api/sheets/import/${projectId}`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ sheetUrl: sheetUrl.trim(), gid: gid.trim() || undefined, columnMapping, autoApprove }),
      });
      const json = await res.json();
      if (!res.ok) { setError(json.error || "Import failed."); setImporting(false); return; }
      setImportResult(json);
      setSuccess(`Successfully imported ${json.imported || json.count || 0} members.`);
      setStep(3);
  ...
  const targetFields = [
    ...
    ...formFields.filter(f => !f.is_system).map(f => ({ key: f.field_key, label: f.label, system: false })),
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => navigate(`/org/${slug}/project/${projectId}`)}
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Google Sheets Import</h1>
            <p className="text-sm text-slate-400">{project?.name || "Project"}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          {[
            ...
            <div key={n} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${step >= n ? "bg-indigo-600/20 text-indigo-400 border border-indigo-500/30" : "bg-slate-800/50 text-slate-500 border border-slate-700/30"}`}>
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${step > n ? "bg-emerald-500 text-white" : step === n ? "bg-indigo-500 text-white" : "bg-slate-700 text-slate-500"}`}>{step > n ? "\u2713" : n}</span>
              {label}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="bg-slate-800/60 backdrop-blur border border-slate-700/50 rounded-2xl p-6">
            <h2 className="text-lg font-semibold text-white mb-1">Google Sheet URL</h2>
            <p className="text-sm text-slate-400 mb-4">Paste the URL of a <strong>publicly shared</strong> Google Sheet. The sheet must be accessible via &quot;Anyone with the link&quot;.</p>

            <div className="space-y-3">
              <div>
                <label className="block text-sm text-slate-300 mb-1">Sheet URL <span className="text-red-400">*</span></label>
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1">Sheet Tab (gid) <span className="text-slate-500 text-xs">optional, default first tab</span></label>
                <input type="text" value={gid} onChange={(e) => setGid(e.target.value)} placeholder="0"
              ...
              <button onClick={handleFetchSheet} disabled={fetching || !sheetUrl.trim()}
                ...
                {fetching ? (
                  <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Fetching...</span>
                ) : "Fetch & Preview"}
              </button>
            </div>
            ...
            <div className="mt-6 p-4 bg-slate-900/40 border border-slate-700/30 rounded-xl">
              <p className="text-sm font-medium text-slate-300 mb-2">{"\uD83D\uDCA1"} How it works</p>
              <ol className="text-xs text-slate-400 space-y-1 list-decimal list-inside">
                <li>Make your Google Sheet public (Share {"\u2192"} Anyone with the link {"\u2192"} Viewer)</li>
                <li>Paste the URL above and click &quot;Fetch &amp; Preview&quot;</li>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/60 backdrop-blur border border-slate-700/50 rounded-2xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-400">{totalRows} rows found &middot; {sheetHeaders.length} columns</p>
                </div>
                <button onClick={resetAll} className="text-sm text-slate-400 hover:text-white transition cursor-pointer">{"\u2190"} Change Sheet</button>
              </div>

              {/* Mapping table */}
              <div className="space-y-2">
                {sheetHeaders.map((header) => (
                  <div key={header} className="flex items-center gap-3 bg-slate-900/40 border border-slate-700/30 rounded-lg p-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white font-medium truncate">{header}</p>
                      <p className="text-xs text-slate-500 truncate">
                        {previewRows.length > 0 && previewRows[0][header] ? `e.g. "${previewRows[0][header]}"` : "No preview data"}
                      </p>
                    </div>
                    <span className="text-slate-600">{"\u2192"}</span>
                    <select value={columnMapping[header] || ""} onChange={(e) => updateMapping(header, e.target.value)}
                      ...
                      {targetFields.map(f => (
                        <option key={f.key} value={f.key} disabled={usedFieldKeys.has(f.key) && columnMapping[header] !== f.key}>
                          {f.label}{f.system ? " *" : ""}
                        </option>
                      ))}
                    </select>
              ...
              <p className="text-xs text-slate-500 mt-3">
                {Object.keys(columnMapping).length} of {sheetHeaders.length} columns mapped
                {!Object.values(columnMapping).includes("name") && <span className="text-amber-400 ml-2">{"\u26A0"} Name column not mapped</span>}
              </p>
            </div>
            ...
            {previewRows.length > 0 && (
              <div className="bg-slate-800/60 backdrop-blur border border-slate-700/50 rounded-2xl p-5 overflow-hidden">
                <h3 className="text-sm font-semibold text-white mb-3">Preview (first {previewRows.length} rows)</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-slate-700/50">
                        {sheetHeaders.map(h => (
                          <th key={h} className={`text-left px-3 py-2 font-medium ${columnMapping[h] ? "text-indigo-400" : "text-slate-500"}`}>
                            {h}
                            {columnMapping[h] && <span className="ml-1 text-[10px] text-emerald-400">({columnMapping[h]})</span>}
                          </th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {previewRows.map((row, i) => (
                        <tr key={i} className="border-b border-slate-800/50">
                          {sheetHeaders.map(h => (
                            <td key={h} className={`px-3 py-2 truncate max-w-[200px] ${columnMapping[h] ? "text-slate-300" : "text-slate-600"}`}>
                              {row[h] || ""}
                            </td>
                          ))}
                        </tr>
            ...
            <div className="bg-slate-800/60 backdrop-blur border border-slate-700/50 rounded-2xl p-5">
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <input type="checkbox" checked={autoApprove} onChange={(e) => setAutoApprove(e.target.checked)} className="accent-indigo-500 w-4 h-4" />
                  <span className="text-sm text-slate-300">Auto-approve imported members</span>
                </label>
                <button onClick={handleImport} disabled={importing || Object.keys(columnMapping).length === 0}
          ...
          <div className="space-y-4">
            <div className="bg-slate-800/60 backdrop-blur border border-slate-700/50 rounded-2xl p-6 text-center">
              ...
              <p className="text-slate-400">
                {importResult.imported || importResult.count || 0} members imported
            ...
            {importResult.validationErrors && importResult.validationErrors.length > 0 && (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5">
                <h3 className="text-sm font-semibold text-amber-400 mb-2">{"\u26A0"} Validation Issues ({importResult.validationErrors.length})</h3>
                <div className="max-h-48 overflow-y-auto space-y-1">
                  {importResult.validationErrors.slice(0, 50).map((e, i) => (
                    <p key={i} className="text-xs text-amber-300/80">Row {e.row}: {e.message || e.error || JSON.stringify(e)}</p>
                  ))}
                  {importResult.validationErrors.length > 50 && (
                    <p className="text-xs text-amber-400">...and {importResult.validationErrors.length - 50} more</p>
                  )}
                </div>
              </div>
            )}
