 ...
 * Bulk Controller
 * ───────────────
 * Handles bulk import, generation, and status operations.
 *
 * Extracted from bulkRoutes.js to follow the controller pattern
 * used throughout the codebase (thin routes → controller → services).
 */
...
/**
 * POST /api/bulk/import/:projectId
 * Import members from an array (Google Sheets data).
 */
const importMembers = async (req, res, next) => {
  try {
    ...
    const rows = members.map((m) => ({
      ...
      status: "approved", // Bulk imports are auto-approved (admin action)
...
/**
 * POST /api/bulk/generate/:projectId
 * Generate all cards for a bulk project. Deducts tokens and auto-refunds
 * if fewer cards are generated than expected (race condition handling).
 */
...
/**
 * GET /api/bulk/status/:projectId
 * Get bulk generation status for a project.
 */
...
module.exports = { importMembers, generateCards, getStatus };
